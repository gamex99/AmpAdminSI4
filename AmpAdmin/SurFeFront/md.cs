...
namespace SurFeFront
{
    public partial class md : Form
    {
        ...
        private void emitirInformeVentasToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            // string sql = "SELECT \r\n    [barcodebaja] as BarCode,\r\n    [cantidadbaja] as CantidadBaja,\r\n    [motivo] as Baja,\r\n   \r\n    u.nombre as Nombre,\r\n    u.apellido as Apellido\r\nFROM [dbo].[motivoBajaStock] AS mb\r\nINNER JOIN [dbo].[usuarios] AS u ON mb.operador = u.id";
            string sql = "SELECT TOP (1000)\r\n    f.id_factura,\r\n    c.razon_social,\r\n    td.descripcion,\r\n    f.fecha,\r\n    f.total\r\nFROM [AmpAdmin].[dbo].[factura] AS f\r\nJOIN [AmpAdmin].[dbo].[cliente] AS c\r\n    ON f.id_cliente = c.id_cliente\r\nJOIN [AmpAdmin].[dbo].[tipo_factura] AS td\r\n    ON f.tipo_documento = td.id;";
            SqlCommand comando = new SqlCommand(sql, connection);
...
<head>
    ...
    <style>
        ...
        thead {
            background-color: #3498db;
            color: white;
            display: table-header-group; /* 🔹 Cabecera repetida */
        }
...
<body>
    <div class='container'>
        <div class='header'>
            <img src='./logo pp1 carpeta 2023.png' />
            <h3>INFORME</h3>
            <p>@tipoinfo</p>
        </div>
        ...
        <div class='footer'>
            <p>Informe generado automáticamente - @DateTime.Now.ToString(""dd/MM/yyyy HH:mm"")</p>
        </div>
    </div>
</body>
</html>";
            ...
            lector.Close();
            PaginaHTML_Texto = PaginaHTML_Texto.Replace("@FILAS", filas);
            PaginaHTML_Texto = PaginaHTML_Texto.Replace("@tipoinfo", " VENTAS REALIZADAS");
            using (FileStream stream = new FileStream(rutaArchivoPDF, FileMode.Create))
            {
                //Creamos un nuevo documento y lo definimos como PDF
                Document pdfDoc = new Document(PageSize.A4, 25, 25, 25, 25);
